...
RETRY_DELAY_S = 3       # délai avant retry en cas d’erreur réseau
...
def fetch_page(page: int) -> Dict:
    ...
    resp.raise_for_status()          # soulève une exception si code HTTP != 200
    return resp.json()

def main() -> None:
    page = 0
    writer = None                    # sera initialisé avec les entêtes la 1ʳᵉ fois
    with open(CSV_FILE, "w", newline="", encoding="utf-8") as f:
        while True:
            ...
            # ───────── Parcours des tokens ─────────
            for token in items:
                # Variables individuelles (optionnel mais demandé)
                id_           = token["id"]
                chain         = token["chain"]
